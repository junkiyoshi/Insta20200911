...
void ofApp::setRingToMesh(ofMesh& face_target, ofMesh& frame_target, glm::vec3 location, float radius, float width, float height, int deg_start, int deg_end) {
	...
	for (int deg = deg_start; deg <= deg_end; deg += 1) {
		...
		auto noise_value = ofNoise(glm::vec3(radius * cos(deg * DEG_TO_RAD) * 0.003, radius * sin(deg * DEG_TO_RAD) * 0.003, ofGetFrameNum() * 0.008));
		auto z = 0;
		if (noise_value > 0.65) { z = ofMap(noise_value, 0.65, 1.0, 0, 250); }
		if (noise_value < 0.35) { z = ofMap(noise_value, 0.35, 0, 0, -250); }
		...
		vertices.push_back(glm::vec3((radius + width * 0.5) * cos((deg + 1) * DEG_TO_RAD), (radius + width * 0.5) * sin((deg + 1) * DEG_TO_RAD), height * -0.5 + z));
		vertices.push_back(glm::vec3((radius + width * 0.5) * cos((deg + 1) * DEG_TO_RAD), (radius + width * 0.5) * sin((deg + 1) * DEG_TO_RAD), height * 0.5 + z));
		vertices.push_back(glm::vec3((radius + width * 0.5) * cos(deg * DEG_TO_RAD), (radius + width * 0.5) * sin(deg * DEG_TO_RAD), height * 0.5 + z));
